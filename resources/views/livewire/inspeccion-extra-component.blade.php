        ...
        <div class="max-w-5xl m-auto bg-white rounded-lg shadow-md my-4 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-3 mb-4 items-stretch">    
                ...
                <div class="md:col-span-3 grid grid-cols-1 gap-2">                    
                    ...
                    <div class="flex items-center gap-2 bg-white border border-gray-200 rounded-lg px-2 py-1">
                        <i class="fas fa-user-tag text-gray-400 text-[10px]"></i>
                        <select wire:model="vendedor_id" class="w-full text-[10px] font-semibold py-1 border-none focus:ring-0 uppercase">
            ...
            --}}
            <div class="grid grid-cols-1 md:grid-cols-12 gap-3 mb-4 items-stretch font-sans">    
                <!-- Área de Pagos (Col-span-5) -->
                <div class="md:col-span-5 bg-gray-50 p-2 rounded-lg border border-gray-200 shadow-sm">
                    <div class="flex justify-between items-center mb-2 px-1">
                        <x-label value="Pagos S/ (Precio):" class="text-[10px] font-bold text-gray-500" />
                        <button type="button" wire:click="agregarPago" 
                            class="text-[10px] bg-indigo-600 text-white px-2 py-0.5 rounded hover:bg-indigo-700 transition-colors shadow-sm">
                            + Agregar
                        </button>
                    </div>                    
                    <div class="space-y-2 max-h-[130px] overflow-y-auto pr-1">
                        @foreach($pagos_multiples as $index => $pago)
                            <div class="flex gap-2 items-center" wire:key="pago-{{ $index }}">
                                <!-- Select de Jetstream (Personalizado para ser pequeño) -->
                                <x-select wire:model="pagos_multiples.{{ $index }}.metodo_pago" 
                                    class="flex-1 text-[11px] py-1 border-gray-300">
                                ...
                                <div class="relative w-28">
                                    <span class="absolute left-2 top-1/2 -translate-y-1/2 text-[10px] font-bold text-green-600">S/</span>
                                    <!-- Input de Jetstream -->
                                    <x-input type="number" step="0.1" 
                                        wire:model.live="pagos_multiples.{{ $index }}.monto" 
                                        wire:change="actualizarMontoTotal"
                                        class="w-full pl-6 pr-3 py-2 text-xs font-bold text-green-700 text-right border-gray-300" 
                                        placeholder="0.00" />
                                </div>
                ...
                <!-- Bloque Central: Gestión (Col-span-4) -->
                <div class="md:col-span-4 grid grid-cols-2 gap-2 bg-gray-50 p-2 rounded-lg border border-gray-200 shadow-sm">                  
                    <!-- Comisión (Fila 1 - Col 1) -->
                    <div class="flex flex-col">
                        <x-label value="Comisión S/:" class="text-[9px] ml-1 mb-0.5 text-gray-400" />
                        <x-input type="number" wire:model="comision_monto" 
                            class="w-full text-xs font-semibold py-1 text-right border-gray-300" 
                            placeholder="0.00" />
                    </div>
                    <!-- Certificado (Fila 1 - Col 2) -->
                    <div class="flex flex-col">
                        <x-label value="N° Certificado:" class="text-[9px] ml-1 mb-0.5 text-gray-400" />
                        <x-input type="text" wire:model="numero_certificado" 
                            class="w-full text-xs font-bold text-indigo-700 py-1 border-gray-300 uppercase" 
                            placeholder="Certificado..." />
                    </div>
                    <!-- Vendedor (Fila 2 - Ocupa las 2 columnas) -->
                    <div class="flex flex-col col-span-2">
                        <x-label value="Vendedor:" class="text-[9px] ml-1 mb-0.5 text-gray-400" />
                        <x-select wire:model="vendedor_id" 
                            class="w-full text-[10px] font-semibold py-1 border-gray-300 uppercase">
                            <option value="">-- Sin Vendedor --</option>
                            @foreach($vendedores as $vend)
                                <option value="{{ $vend->id }}">{{ $vend->nombre }}</option>
                            @endforeach
                ...
                <!-- Bloque Final: Resumen (Col-span-3) -->
                <div class="md:col-span-3 flex flex-col gap-2 p-2 bg-gray-50 rounded-lg border border-gray-200 shadow-sm">                   
                    <!-- Vigencia -->
                    <div class="flex flex-col">
                        <x-label value="Vigencia (Meses):" class="text-[9px] ml-1 mb-0.5 text-gray-400" />
                        <x-select wire:model="vigencia_meses" class="w-full text-xs font-semibold py-1 border-gray-300">
                            <option value="6">6 Meses</option>
                            <option value="12">12 Meses</option>
                        </x-select>
                    </div>

                    <!-- Total Cobrar (Estilizado pero manteniendo jerarquía) -->
                    <div class="bg-indigo-50 border border-indigo-100 rounded-lg p-3 flex justify-between items-center mt-auto shadow-inner">
                        <x-label value="Total Cobrar" class="text-[10px] font-black text-indigo-900 uppercase" />
                        <div class="text-xl font-black text-indigo-700 tracking-tight">
                            S/{{ number_format($monto_total, 2) }}
                        </div>
                    </div>
                </div>
